...
def test_closed_loop_traverse():
    ...
    assert result.adjusted[0] == (1000.0, 1000.0)

    # Last point coincides with first
    assert math.isclose(result.adjusted[-1][0], 1000.0, abs_tol=1e-10)
    assert math.isclose(result.adjusted[-1][1], 1000.0, abs_tol=1e-10)
...
def test_corrections_proportional():
    ...
    coords = [
        ...
        (100.0, 0.0),
        (50.0, 86.6025),
        (0.04, 0.03),  # misclosure back to origin
    ]
    result = compass_rule(coords)
    ...
    assert math.isclose(
        result.adjusted[1][0], 100.0 - 0.04 * cum1 / total, abs_tol=1e-6
    )
    assert math.isclose(
        result.adjusted[2][0], 50.0 - 0.04 * cum2 / total, abs_tol=1e-6
    )
    # Last point matches start
    assert math.isclose(result.adjusted[-1][0], 0.0, abs_tol=1e-10)
    assert math.isclose(result.adjusted[-1][1], 0.0, abs_tol=1e-10)
...
def test_fixed_endpoint_traverse():
    ...
    known_end = (1100.0, 1100.0)

    result = compass_rule(coords, known_end=known_end)

    assert math.isclose(result.misclosure_n, 0.0, abs_tol=1e-10)
    assert math.isclose(result.misclosure_e, 0.03, abs_tol=1e-10)
    assert math.isclose(result.adjusted[-1][0], 1100.0, abs_tol=1e-10)
    assert math.isclose(result.adjusted[-1][1], 1100.0, abs_tol=1e-10)
...
def test_zero_misclosure():
    ...
    for orig, adj in zip(coords, result.adjusted):
        assert math.isclose(orig[0], adj[0], abs_tol=1e-10)
        assert math.isclose(orig[1], adj[1], abs_tol=1e-10)


def test_too_few_points():
    """Compass rule needs at least 3 points."""
    with pytest.raises(ValueError):
        compass_rule([(0, 0), (1, 1)])

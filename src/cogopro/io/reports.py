"""HTML report generation for COGOpro commands.
...
from cogopro.solvers.horizontal_curve import CurveElements
...
def _html_wrap(title: str, body: str) -> str:
    ...
    return (
        ...
        "</head>\n<body>\n"
        f"<h1>{_esc(title)}</h1>\n"
        f"<p class=\"subtitle\">Generated {timestamp} by COGOpro</p>\n"
        f"{body}\n"
        f"<footer>COGOpro &mdash; {_esc(timestamp)}</footer>\n"
        "</body>\n</html>\n"
    )

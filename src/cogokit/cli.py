"""cogokit command-line interface."""
...
from cogokit.cogo.traverse import traverse as _traverse
from cogokit.core import Angle, Point
from cogokit.io import export_dxf, export_kml, read_points
from cogokit.solvers.horizontal_curve import solve_curve
from cogokit.surveying.workflow import TraverseWorkflow

app = typer.Typer(help="cogokit — coordinate geometry & surveying toolkit")
...
def config_set(
    ...
    from cogokit.config import get_config, save_config
...
def zones(
    ...
    from cogokit.geodetic.state_plane import get_zone, list_zones
    ...
    if epsg is not None:
        ...
        if report is not None:
            from cogokit.io.reports import report_zones, write_report
...
def _parse_crs(crs_str: str):
    ...
    try:
        ...
        from cogokit.geodetic.ellipsoid import CLARKE_1866, GRS80, NAD83, WGS84
    except ImportError:
        return None
...
def convert(
    ...
    try:
        from cogokit.core.crs import transform_job
    except ImportError:
    ...
    if report is not None:
        from cogokit.io.reports import report_convert, write_report
...
def export(
    ...
    if report is not None:
        from cogokit.io.reports import report_export, write_report
...
def traverse_run_cmd(
    ...
    if report is not None:
        ...
        from cogokit.io.reports import report_traverse_run, write_report

...
The actual projection math is delegated to :mod:`cogokit.geodetic.projections`.
"""
...
if TYPE_CHECKING:
    ...
    from cogokit.geodetic.ellipsoid import Ellipsoid
...
def transform_point(point: Point, from_crs: CRS, to_crs: CRS) -> Point:
    ...
    if from_crs.kind == "geodetic":
        ...
        from cogokit.geodetic.projections import projection_inverse
...
def transform_job(job: Job, to_crs: CRS, *, from_crs: CRS | None = None) -> Job:
    ...
    """
    from cogokit.core.job import Job as JobClass
